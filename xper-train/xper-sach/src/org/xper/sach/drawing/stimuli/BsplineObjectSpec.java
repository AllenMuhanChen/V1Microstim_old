...
public class BsplineObjectSpec {
	...
	// moved occluder spec here from BsplineObject
	OccluderSpec occluderSpec = null; // new OccluderSpec();

		
	// JK 18 April 2016 
	double rgbaVec[] = {0.0, 0.0, 0.0, 1.0};
	...
	static {
		...
		// JK 10 August 2016
		s.alias("occluderSpec", OccluderSpec.class);
	}
...
//		 return ll;
	...
	public static BsplineObjectSpec fromXml(String xml) {
		...
		//System.out.println("BsplineObjectSpec fromXml() : " + xml	);
		 // JK need to explicitly set occluder to null
// ??? JK 13 Oc 2016		 g.clearOccluder();  
...
//		SizeGrowthValues[11] =  -0.6;
	...
	public BsplineObjectSpec(BsplineObjectSpec d) {
		...
		int len = d.rgbaVec.length;
		rgbaVec = new double[len]; 
		System.arraycopy(d.rgbaVec, 0, rgbaVec, 0, len);
		
		if(d.isOccluded()){
			occluderSpec = new OccluderSpec();
			occluderSpec = d.occluderSpec;
		} else {
			occluderSpec = null;			
		}
...
//		}
	...
	public void setCategory(int category, boolean shouldUseColorCues, boolean shouldUseSizeCues) {
		...
		if(shouldUseColorCues){
			if(category > -1 && category < BsplineObject.getNumCategories()){
				...
				setRgbaVec(Colors[category * n + 0] * fader, Colors[category * n + 1] * fader, Colors[category * n + 2] * fader, Colors[category * n + 3]);
			}
			//System.out.println("BSOSpec::setCategory(int, bool) : " + category + " : color == " + getRedVal() + ", " + getGreenVal() + ", " + getBlueVal() + ", " + getAlphaVal());
		}
		else {
			setRgbaVec(0.0, 0.0, 0.0, 1.0);
		}
	...
	// JK 20 April 2016
	public void setRgbaVec(double redVal, double greenVal, double blueVal, double alphaVal){
		if(redVal <= 1.0 && redVal >= 0.0){
			this.rgbaVec[0] = redVal;
		}
		if(greenVal <= 1.0 && greenVal >= 0.0){
			this.rgbaVec[1] = greenVal;
		}
		if(blueVal <= 1.0 && blueVal >= 0.0){
			this.rgbaVec[2] = blueVal;
		}
		if(alphaVal <= 1.0 && alphaVal >= 0.0){
			this.rgbaVec[3] = alphaVal;
		}
	}
	...
	// JK 17 May 2016
	public void setOccluder(OccluderSpec occ){
		occluderSpec = new OccluderSpec();
		
		occluderSpec = occ;
	}
	
	public OccluderSpec getOccluderSpec() {
		if(occluderSpec == null){
			occluderSpec = new OccluderSpec();
			//System.out.println("BsplineObjectSpec::getOccluderSPec() : created a fresh one ...");
		}
		return occluderSpec;
	}
	
	public void setOccluderAlphaGain(double alphaGain){

		this.occluderSpec.setAlphaGain((float) alphaGain);
	}
	...
	public boolean isOccluded() {
		if(occluderSpec != null){
			//System.out.println("BsplineObjectSpec::isOccluded() : isOccluded ...");
			return true;
		} else {
			//System.out.println("BsplineObjectSpec::isOccluded() : **** not Occluded ****");
			return false;
		}
