...
public class OriginalBsplineObject implements Drawable {
	...
	boolean drawCtrlPts = true;				// (for debugging)
	boolean drawNodes = false;					// (for debugging)
	boolean drawNums = false;					// (for debugging)
	boolean printStimSpecs = true;				// (for debugging) 
	double jointAngleCurvFactor = 0.3;//0.3;	// from 0 to 1, roundedness of joint angles, *** need to re-work this, want to ensure fixed medial-axis with changes in width, etc. ***
	...
	static public enum StimCategories { SEVEN_h, Y_h, downL_downT, I_downT, SEVEN_t, Y_t, downL_J, I_J, downL_h, I_h, SEVEN_downT, Y_downT, downL_t, I_t, SEVEN_J, Y_J }; 
...
//		
		// pinching!
		s.addLimbSpec(new LimbSpec(-1,4,1,1,0,new double[]{1,1,1,1,1,1,1,1},true));
		s.addLimbSpec(new LimbSpec( 1,3,1,0,new double[]{1,1,1,1},false));
		s.addLimbSpec(new LimbSpec( 0,3,1,180,new double[]{2,2,1,1},false));
...
//		testWindow.testDraw();				// draw object
	...
	public OriginalBsplineObject() {
		super();
	}
	
	void createObj() {	// chooses which createObj routine to run
...
//			// need StimSpecID for this!
		...
		for (int n = 0; n < objCtrlPts.length; n++) { 
			double[][] objCtrlPts_rotated = SachMathUtil.deepCopyMatrix(objCtrlPts);
			objCtrlPts_rotated = SachMathUtil.rotate2D(global_ori, objCtrlPts_rotated);
			spline.addPoint((objCtrlPts_rotated[n][0]-xDiff)*size,(objCtrlPts_rotated[n][1]-yDiff)*size);
...
//		int limbsToMake = SachMathUtil.randRange(lim_maxNumLimbs,lim_minNumLimbs);	// rand # limbs between minNumLimbs and maxNumLimbs
	...
	void createMorphObj() {
		...
		MorphTypes morphType = chooseMorphType();
		...
		do {	// while reMorphFlag = true; (this will try to apply the same type of morph if it fails)
			...
			case CANONICAL: // __canonical__ (switches to a canonical medial axis view where surface curvature is minimized)
				...
				for (int nLimb=0;nLimb<localNumLimbs;nLimb++) {
	...
	void firstLimb(double l, double w1,double w2,double ori,double[] ctr,double[] curv,boolean isSmoother,boolean isSmoother2) {
...
//		}
	...
	boolean addLimb(int nodeID,double l,double w,double ori,double[] curv,boolean isSmoother) { 
...
//			limbs.add(limbID+1, new LimbSpec(nodeID,half_length,limbs.get(limbID).getWidth(),rescaleOri(old_ori+180),new_curv,limbs.get(limbID).isSmoother()));
	...
	boolean addRandLimbWithSplit(int nodeID, boolean doSplit) {
	...
	boolean addRandomLimb(int nodeID) {
...
//				SimpleText.drawString(Integer.toString(n),(float)spline.pt[n].x+1,(float)spline.pt[n].y+1);
	...
	public void drawNodesMethod(Context context) {
		for (int m=0;m<numNodes;m++) {
			...
			CircleObject s = new CircleObject(0.5,(nodeXY[0]-xDiff)*size+xPos,(nodeXY[1]-yDiff)*size+yPos,0,1,1,0);		// only add x,y position changes when drawing!
...
//		yPos = yPos+SachMathUtil.randRange(1,-1);
		...
		if (doRand) {
			pL = 0.1;	//0.4; TODO: these limit how much lengths and width are allowed to change!						
			pW = 0.0;	//0.4;
		}
