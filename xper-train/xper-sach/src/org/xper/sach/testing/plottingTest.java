...
//		double[][] dXdY = {{1,10},{1,20},{1,5},{1,0},{1,15},{1,30},{1,10},{1,25}};
		...
		double[] cats = {0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5};
		double[] stdevs = {4,6,3,3,2,4,1,4};
...
//		plot1.addHistogramPlot("hist", z, 30);
		...
		plot1.setAxisLabels("category","response");
		//plot1.setAxisLabel(0, "category");
		plot1.getAxis(0).setLabelPosition(0.5, -0.15);

		//plot1.setAxisLabel(1, "response");
//		plot1.getAxis(1).setLabelPosition(-0.05, 1.1);
		plot1.getAxis(1).setLabelPosition(-0.15, 0.5);
		plot1.getAxis(1).setLabelAngle(-Math.PI/2);
		plot1.removePlotToolBar();
		
		plot1.addHistogramPlot("bar", XY, dX);
		plot1.addLinePlot("line",cats,stdevs);

		
		// customize X axe
        // rotate light labels
        //plot1.getAxis(0).setLightLabelAngle(-Math.PI / 4);
