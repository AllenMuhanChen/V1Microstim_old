...
public class SachMathUtil {
	...
	public static void initialize() {
		...
		System.out.println("SachMathUtil initialized seed");
		rand.setSeed(seed);
	}
	...
	// JK 31 August 2016
	//    Modified all randomizers to use Random class methods instead of Math.random() e.g.
	//    rand.nextDouble()  replaces Math.random()
	//    This allows setting & storing the seed.  Additionally, the Random methods are 
	...
	public static int randRange(int upper, int lower) {
		...
		int randInt = (int) (rand.nextDouble() * range) + lower;
	...
	public static int[] randRange(int upper, int lower, int numRands) {
		...
		for (int n=0;n<numRands;n++) {
			randArray[n] = (int)(rand.nextDouble()*((upper-lower)+1))+lower;
		}
		return randArray;
	}
	...
	public static double[] randRange(double upper, double lower,int numRands) {
		...
		for (int n=0;n<numRands;n++) {
			randArray[n] = rescaleValue(rand.nextDouble(),upper,lower);
		}
		return randArray;
	}
	...
	public static List<Integer> randUniqueRange(int upper, int lower, int numRands){
		...
		// remove some
		for (int n = 0; n < (size - numRands); n++) {
			x = rand.nextInt(vals.size());
			vals.remove(x);
		}
	...
	public static double randUshapedPoly(double vMin,double vMax, double ctr, double bias){
		...
		boolean isGenSideLow = false;
		if (ctr<vMin+vDiff/20){
			ctr = vMin;
			isGenSideLow = false; 
		}
		if (ctr>vMax-vDiff/20){
			ctr = vMax;
			isGenSideLow = true;
		}
		
		if ((ctr>vMin) && (ctr<vMax)){
			isGenSideLow = rand.nextBoolean();
		}
		
		double u = rand.nextDouble(); // generate u from uniform distribution
		...
		if (isGenSideLow) {
			return ctr-x*(ctr-vMin);
		}
		else {
			return ctr+x*(vMax-ctr);
		}
	}
	...
	public static int[] intersectElement(int[] s, int[] r) {
	    List<Integer> result = new ArrayList<Integer> ();
	    for (int i : s) {
	    	for (int i2 : r){
	    		if (i == i2) {
	...
	public static double[][] subtractVectors(double[][] M,double[] V) {
		double[][] res = SachMathUtil.deepCopyMatrix(M);
