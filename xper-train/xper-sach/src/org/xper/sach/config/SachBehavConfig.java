...
public class SachBehavConfig {
	...
	public SachExptScene taskScene() {
		...
		// JK 27 July adding optional fix color  
	...
	// JK 27 July 2016
	//  adding a dbase color parameter, xper_fixation_point_optional_color
	@Bean
	public FixationPoint experimentFixationPoint() {
		FixationPoint f = new FixationPoint ();
		f.setColor(xperFixationPointOptionalColor());
		f.setFixationPosition(classicConfig.xperFixationPosition());
		f.setSize(classicConfig.xperFixationPointSize());
		return f;
	}
	
	@Bean(scope = DefaultScopes.PROTOTYPE)
	public RGBColor xperFixationPointOptionalColor() {
		RGBColor color = new RGBColor();
		color.setRed(Float.parseFloat(baseConfig.systemVariableContainer().get("xper_fixation_point_optional_color", 0)));
		color.setGreen(Float.parseFloat(baseConfig.systemVariableContainer().get("xper_fixation_point_optional_color", 1)));
		color.setBlue(Float.parseFloat(baseConfig.systemVariableContainer().get("xper_fixation_point_optional_color", 2)));
		return color;
	}
	/////
	...
	public SachExperimentState experimentState() {
		...
		// JK 25 July 2016
		state.setMaintainHoldTime(sachConfig.xperMaintainHoldTime());
