...
//		win.setPixelFormat(new PixelFormat(0, 8, 1));
	...
	public SachExperimentState experimentState() {
		...
		// AWC set to "true" July2017
		state.setRepeatTrialIfEyeBreak(true); // TODO: set to true if I allow eye movements during last stim (less false breaks!)
...
//		source.setUngetBehavior(UngetPolicy.TAIL);
	...
	// JK 25 July 2016
	@Bean(scope = DefaultScopes.PROTOTYPE)
	public Long xperMaintainHoldTime() {
		return Long.parseLong(baseConfig.systemVariableContainer().get("xper_maintain_hold_time", 0));
	}
