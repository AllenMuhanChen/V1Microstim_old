...
public class DbUtil {
	...
	public void writeEStimSpec(EStimSpec e) {
		JdbcTemplate jt = new JdbcTemplate(dataSource);
		jt.update("insert into EStimSpec (id, chan, trig_src, num_pulses, pulse_train_period, post_stim_refractory_period, stim_shape, stim_polarity, d1, d2, dp, a1, a2, pre_stim_amp_settle, post_stim_amp_settle, maintain_amp_settle_during_pulse_train, post_stim_charge_recovery_on, post_stim_charge_recovery_off) values (?, ?, ?, ? ,? ,? ,? ,? ,? ,? ,?, ?, ?, ?, ?, ?, ?, ?)",
				new Object[] { e.get_id(), e.get_chan(), e.get_trig_src(), e.get_num_pulses(), e.get_pulse_train_period(), e.get_post_stim_refractory_period(), e.get_stim_shape(), e.get_stim_polarity(), e.get_d1(), e.get_d2(), e.get_dp(), e.get_a1(), e.get_a2(), e.get_pre_stim_amp_settle(), e.get_post_stim_amp_settle(), e.get_maintain_amp_settle_during_pulse_train(), e.get_post_stim_charge_recovery_on(), e.get_post_stim_charge_recovery_off()});
	}
	//AC
	public EStimSpec readEStimSpec(long estimId) {
		...
		return jt.queryForObject(
				...
				new ParameterizedRowMapper<EStimSpec> () {
					public EStimSpec mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						...
						e.set_maintain_amp_settle_during_pulse_train(rs.getInt("maintain_amp_settle_during_pulse_train"));
	...
	public BlockSpec readBlockSpec(long blockId) {
		...
		return jt.queryForObject(
				...
				new ParameterizedRowMapper<BlockSpec> () {
					public BlockSpec mapRow(ResultSet rs, int rowNum)
							throws SQLException {
	...
	public void writeTaskToDo(long taskId, long stimId, int estimId, long xfmId, long genId) {
		JdbcTemplate jt = new JdbcTemplate(dataSource);
		jt.update("insert into TaskToDo(task_id, stim_id, estim_id, xfm_id, gen_id) values (?, ?, ?, ?, ?)", 
						new Object[] { taskId, stimId, estimId, xfmId, genId });
		//AC edits- added estim_id
	}
