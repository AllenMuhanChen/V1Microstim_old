...
CREATE TABLE AcqData (
  ...
  PRIMARY KEY  (tstamp)
) TYPE=MyISAM MAX_ROWS=4294967295 AVG_ROW_LENGTH=16384 COMMENT='Timestamp: when the first data record is saved.';

CREATE TABLE AcqSession (
  start_time bigint(20) NOT NULL default '0',
  stop_time bigint(20) NOT NULL default '0',
  PRIMARY KEY  (start_time)
) TYPE=MyISAM COMMENT='Timestamp: start and stop of each trial.';
...
CREATE TABLE TaskDone (
  ...
  PRIMARY KEY  (tstamp)
) TYPE=MyISAM COMMENT='Timestamp: when the stim is shown.';
